...
class Player {
    ...
    submitAnnotations(e) {
        ...
        DataSources.annotations.save(this.videoId, this.annotations, this.metrics, window.mturk).then((response) => {
            ...
            window.Labelbox.fetchNextAssetToLabel().then(() => {
                // TODO: ew
                // wait for label to save because no promise notifying resolution
                // reload the page because Player cannot be re-used
